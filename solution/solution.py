...
def _clamp_score(score: float) -> float:
    """Keep heuristic metric values inside the expected 0-1 range."""
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the AI response using the rubric below. "
            "Return JSON where each rubric criterion maps to a numeric score "
            "between 0 and 1.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_lines}\n"
        ...
        try:
            ...
            scores_payload = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
            scores: dict[str, float] = {}
            for criterion in rubric:
                value = scores_payload.get(criterion) if isinstance(scores_payload, dict) else None
                if isinstance(value, (int, float)):
                    numeric = float(value)
                    if numeric > 1.0:
                        numeric = numeric / 5.0
                    scores[criterion] = _clamp_score(numeric)
                else:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            first_value = item.get("first_score", item.get("response_a_score"))
            second_value = item.get("second_score", item.get("response_b_score"))
            if isinstance(first_value, (int, float)) and isinstance(second_value, (int, float)):
                first_scores.append(float(first_value))
                second_scores.append(float(second_value))

        average = sum(all_scores) / len(all_scores) if all_scores else 0.0
        positional_bias = False
        if first_scores and second_scores:
            first_wins = sum(1 for first, second in zip(first_scores, second_scores) if first > second)
            positional_bias = first_wins / len(first_scores) > 0.7
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": average(context_recall_scores) if context_recall_scores else None,
            "avg_context_precision": (
                average(context_precision_scores) if context_precision_scores else None
            ),
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_avg_faithfulness = average_metric(new_results, "faithfulness")
        new_avg_relevance = average_metric(new_results, "relevance")
        new_avg_completeness = average_metric(new_results, "completeness")
        baseline_avg_faithfulness = average_metric(baseline_results, "faithfulness")
        baseline_avg_relevance = average_metric(baseline_results, "relevance")
        baseline_avg_completeness = average_metric(baseline_results, "completeness")
        ...
        regressions = [
            metric
            for metric, (new_average, baseline_average) in comparisons.items()
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            ...
            "irrelevant": "Add intent-specific prompt examples to improve answer relevance",
            "incomplete": "Add few-shot examples showing complete answers to improve completeness",
            "off_topic": "Improve routing and query understanding for ambiguous user questions",
            "refusal": "Review refusal guardrails to reduce false refusals on in-scope questions",
            "unknown": "Review uncategorized failures and assign a concrete failure taxonomy label",
        }
        ...
        for failure_type, _count in sorted(
            ...
            suggestions.append(
                suggestions_by_type.get(
                    failure_type,
                    f"Investigate recurring {failure_type} failures and add regression cases",
                )
            )

        general_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add regression tests for the lowest-scoring benchmark cases",
            "Review retrieved contexts before generation to detect missing evidence",
        ]
